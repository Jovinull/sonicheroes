...
struct bad_exception_object {
	void* vtable;
};

#define RETURN_ADDRESS               4
#define DTORCALL_COMPLETE(dtor, obj) (*(void (*)(void*, int))(dtor))((obj), -1)
#define DTORCALL_PARTIAL(dtor, obj)  (*(void (*)(void*, int))(dtor))((obj), 0)
...
static void ExPPC_ThrowHandler(ThrowContext*);
static inline void ExPPC_HandleUnexpected(ThrowContext*, ExceptionInfo*, ex_specification*);
static inline int ExPPC_IsInSpecification(const char*, ex_specification*);
static inline int ExPPC_IsInSpecificationUnexpected(ex_specification*, const char*);
static inline int ExPPC_IsNameInSpecification(ex_specification*, const char*);
static void ExPPC_UnwindStack(ThrowContext*, ExceptionInfo*, void*);
static char* ExPPC_PopStackFrame(ThrowContext*, ExceptionInfo*);
static exaction_type ExPPC_NextAction(ActionIterator*);
extern "C" void ExPPC_FindExceptionRecord(char*, ExceptionInfo*);
...
extern "C" asm void __throw(char* throwtype, void* location, void* dtor)
{
...
static inline int ExPPC_IsInSpecification(const char* extype, ex_specification* spec)
{
	long i;
	long offset;
...
static inline int ExPPC_IsInSpecificationUnexpected(ex_specification* spec, const char* extype)
{
	register ex_specification* cursor = spec;
	register const char* type         = extype;
	long i;
	long offset;

	for (i = 0; i < spec->specs; i++) {
		if (__throw_catch_compare(type, cursor->spec[0], &offset)) {
			return 1;
		}
		cursor = (ex_specification*)((char*)cursor + sizeof(void*));
	}
	return 0;
}

static inline int ExPPC_IsNameInSpecification(ex_specification* spec, const char* extype)
{
	long i;
	register ex_specification* cursor = spec;
	long offset;
	...
	for (i = 0; i < spec->specs; i++) {
		if (__throw_catch_compare(extype, cursor->spec[0], &offset)) {
			return 1;
		}
		cursor = (ex_specification*)((char*)cursor + sizeof(void*));
	}
...
static inline void ExPPC_HandleUnexpected(
    ThrowContext* context, ExceptionInfo* info, ex_specification* unexp)
{
	CatchInfo* catchinfo;
...
static void ExPPC_ThrowHandler(ThrowContext* context)
{
	...
	for (action = ExPPC_CurrentAction(&iter);; action = ExPPC_NextAction(&iter)) {
		switch (action) {
			...
			case EXACTION_CATCHBLOCK:
				if (__throw_catch_compare(context->throwtype,
				        ((ex_catchblock*)iter.info.action_pointer)->catch_type, &offset)) {
					break;
				}
				continue;
			case EXACTION_SPECIFICATION:
	...
	if (action == EXACTION_CATCHBLOCK_32) {
		ex_catchblock_32* catchblock;

		catchblock = (ex_catchblock_32*)iter.info.action_pointer;
...
extern "C" void __unexpected(register CatchInfo* catchinfo)
{
	register ex_specification* specification = (ex_specification*)catchinfo->stacktop;

	try {
		std::unexpected();
	} catch (...) {
		register char* current_type;

		// clang-format off
		asm { lwz current_type, 0x20(r31) }
		// clang-format on

		if (ExPPC_IsInSpecificationUnexpected(specification, current_type))
		{
			throw;
		}
		if (ExPPC_IsNameInSpecification(specification, "!bad_exception!!")) {
			bad_exception_object exception;
			exception.vtable = __vt__Q23std9exception;
			exception.vtable = __vt__Q23std13bad_exception;
			__throw("!std::exception!!std::bad_exception!!", &exception,
			    (void*)__dt__Q23std13bad_exceptionFv);
		}
		if (ExPPC_IsNameInSpecification(specification, "!std::bad_exception!!\0\0")) {
			bad_exception_object exception;
			exception.vtable = __vt__Q23std9exception;
			exception.vtable = __vt__Q23std13bad_exception;
			__throw("!std::exception!!std::bad_exception!!", &exception,
...
__declspec(weak) std::bad_exception::~bad_exception() { }
...
static inline void ExPPC_DestroyLocalPointer(
    ThrowContext* context, const ex_destroylocalpointer* ex)
{
...
static void ExPPC_UnwindStack(ThrowContext* context, ExceptionInfo* info, void* catcher)
{
	exaction_type action;
	...
	for (;;) {
		...
		switch (action & EXACTION_MASK) {
			...
			case EXACTION_ACTIVECATCHBLOCK: {
				...
				catchinfo = (CatchInfo*)(context->FP
...
static char* ExPPC_PopStackFrame(ThrowContext* context, ExceptionInfo* info)
{
	char* SP;
	char* callers_SP;
...
static inline long ExPPC_PopR31(char* SP, ExceptionInfo* info)
{
	double* FPR_save_area;
	long* GPR_save_area;
	int saved_GPRs;
...
extern "C" void ExPPC_FindExceptionRecord(char* returnaddr, ExceptionInfo* result)
{
	...
	result->current_function = fragment->code_start + p->functionoffset;
	result->exception_record = ETI_GetDirectStore(p->eti_field)
