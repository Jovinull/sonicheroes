...
// S20D CURBSTONE's destructor. It restores both vtable pointers, unregisters
// the clump's atomic from wherever it was registered and drops the clump,
// runs the embedded motion base down and then the object base, and hands
// the object back to the heap when the caller asks for it. The object
// embeds no Volume base, only Motion.
//
// The claim is .text 0xBB060 to 0xBB120 and nothing else. The vtable stays
// in each module's data and is renamed to the shared name below; it reads
// no constant, so it owns no rodata.
//
// s20dCurbstoneCreate (rel/s20d_curb_stone_register.cpp) constructs objects
...
// rel/o_s01_hana.cpp and rel/s02_water_dtor.cpp: look up the atomic from the
// clump first, then unregister the atomic/clump pair, then drop the clump
// itself.
//
// stage01D only.
//
...
extern "C" void* s20dCurbstoneVtable[];

extern "C" S20dCurbstone* s20dCurbstoneDtor(S20dCurbstone* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = s20dCurbstoneVtable;
		object->motion.vtable = s20dCurbstoneVtable + 0xB;
