...
// S11 DOOR OBJECT's destructor. It restores both vtable pointers, releases
// two models in a loop (unregistering each from the world's scene before
// dropping it), runs the embedded volume and motion bases down and then the
// object base, and hands the object back to the heap when the caller asks
// for it. The same release-loop shape as rel/ironball_dtor.cpp and
// rel/s12_door_object_dtor.cpp, just two models instead of five.
//
// The claim is .text 0x95F38 to 0x96018 and nothing else. The vtable stays
// in each module's data and is renamed to the shared name below; it reads
// no constant, so it owns no rodata.
//
// s11doorObjectCreate (rel/s11door_object_register.cpp) constructs objects
...
extern "C" void* s11doorObjectVtable[];

static void s11doorObjectReleaseModels(S11DoorObject* object)
{
	s32 i;
...
extern "C" S11DoorObject* s11doorObjectDtor(S11DoorObject* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = s11doorObjectVtable;
		object->motion.vtable = s11doorObjectVtable + 0xB;

		s11doorObjectReleaseModels(object);
