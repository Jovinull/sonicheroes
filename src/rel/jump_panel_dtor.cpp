...
// TObjJumpPanel's destructor. It restores both vtable pointers, unregisters
// the mesh it built from the world's scene and drops it, frees a buffer
// through operator delete, runs the embedded volume and motion bases down
// and then the object base, and hands the object back to the heap when the
// caller asks for it.
//
// The claim is .text 0xC6CC to 0xC7AC and nothing else. The vtable stays in
// each module's data and is renamed to the shared name below; it reads no
// constant, so it owns no rodata.
//
// jumpPanelCreate (rel/jump_panel_register.cpp) constructs objects with the
...
extern "C" void* jumpPanelVtable[];

extern "C" JumpPanel* jumpPanelDtor(JumpPanel* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = jumpPanelVtable;
		object->motion.vtable = jumpPanelVtable + 0xB;
