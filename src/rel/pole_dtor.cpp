...
// TObjPole's destructor. It restores both vtable pointers, unregisters the model
// it built from the world's scene and drops it, runs the embedded volume and
...
//
// The name is read from the layout: the run ends exactly where poleCtor begins.
//
...
extern "C" void* poleVtable[];

extern "C" Pole* poleDtor(Pole* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = poleVtable;
		object->motion.vtable = poleVtable + 0xB;
