...
// TObjLaserFence's destructor. It restores both vtable pointers, runs the
...
//
// The claim is .text 0x0001BED4 to 0x0001BF6C and nothing else. The vtable stays
// in each module's data and is renamed to the shared name below; it reads no
// constant, so it owns no rodata.
//
// The name is read from the layout: the run ends exactly where laserFenceCtor
// begins, which is the destructor-then-constructor order the other stage
// objects also have.
//
// The run is the same in all twelve stage modules that share the engine core,
// checked by normalising the disassembly and comparing across modules.
//
...
extern "C" void* laserFenceVtable[];

extern "C" LaserFence* laserFenceDtor(LaserFence* object, s16 flags)
{
	if (object != NULL) {
