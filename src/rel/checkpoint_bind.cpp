...
// TObjCheckpoint's bind hook. It publishes the object's handle at +0x18 into the
// stage-local slot, raises bit 6 of that handle's flags, and hooks the field
// callback onto it.
//
// The claim is .text 0x00010A38 to 0x00010A94 and nothing else. It reads no
// constant, so it owns no rodata.
//
// The name is read from the layout: the run sits between checkpointCreate and
...
// every module's symbols.txt rather than left as fn_<id>_10A94, because a
...
extern "C" Handle* checkpointBoundHandle;

extern "C" void checkpointFieldCallback(void);

extern "C" void fn_801527A4(Handle* handle, void* callback, void* arg);

extern "C" Checkpoint* checkpointBind(Checkpoint* object, void* arg)
{
	checkpointBoundHandle = object->handle;
	checkpointBoundHandle->flags |= 0x40;

	fn_801527A4(checkpointBoundHandle, (void*)checkpointFieldCallback, arg);
