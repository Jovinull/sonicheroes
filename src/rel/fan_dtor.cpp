...
// TObjFan's destructor. It restores both vtable pointers, unregisters
...
//
// The claim is .text 0x000251E8 to 0x000252B0 and nothing else. The vtable
// stays in each module's data and is renamed to the shared name below; it reads
// no constant, so it owns no rodata.
//
// The name is read from the layout: the run ends exactly where fanCtor
// begins.
//
...
extern "C" void* fanVtable[];

extern "C" Fan* fanDtor(Fan* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = fanVtable;
		object->motion.vtable = fanVtable + 0xB;
