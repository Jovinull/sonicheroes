...
// TObjReel's destructor. It restores both vtable pointers, unregisters
// the model it built from the world's scene and drops it, runs the embedded
// volume and motion bases down and then the object base, and hands the object
// back to the heap when the caller asks for it.
//
// The claim is .text 0x00011B18 to 0x00011BE0 and nothing else. The vtable
// stays in each module's data and is renamed to the shared name below; it reads
// no constant, so it owns no rodata.
//
// The name is read from the layout: the run ends exactly where reelCtor
// begins.
//
...
extern "C" void* reelVtable[];

extern "C" Reel* reelDtor(Reel* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = reelVtable;
		object->motion.vtable = reelVtable + 0xB;
