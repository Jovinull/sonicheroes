...
// S12 DOOR OBJECT's destructor. It restores both vtable pointers, releases
...
// for it. The same release-loop shape as rel/ironball_dtor.cpp, just two
// models instead of five.
//
// The claim is .text 0x94A38 to 0x94B18 and nothing else. The vtable stays
// in each module's data and is renamed to the shared name below; it reads
// no constant, so it owns no rodata.
//
// s12doorObjectCreate (rel/s12door_object_register.cpp) constructs objects
...
extern "C" void* s12doorObjectVtable[];

static void s12doorObjectReleaseModels(S12DoorObject* object)
{
	s32 i;
...
extern "C" S12DoorObject* s12doorObjectDtor(S12DoorObject* object, s16 flags)
{
	if (object != NULL) {
		object->base.vtable   = s12doorObjectVtable;
		object->motion.vtable = s12doorObjectVtable + 0xB;

		s12doorObjectReleaseModels(object);
