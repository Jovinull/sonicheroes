...
// Submits one textured quad, in white.
//
// The translation unit is the single function fn_1_144AC, .text 0x144AC to
// 0x145B4. It is the same routine as fn_2_31F0 in autosaveD's quad_submit.c,
// with the same four vertices in the same order and the same submit call. The
// difference is where the colour comes from: autosaveD reads a colour it is
// given, this one always copies the constant at .rodata 0x1A80, which is
// 0xFFFFFFFF. That constant is named rather than written out, so this unit
// claims no .rodata of its own.
//
// The first parameter is dead - the original clobbers it with the address of
// that constant before reading anything - but it is kept because every caller
// passes it and fn_2_31F0 has it too.
...
extern "C" const Color lbl_1_rodata_1A80;

extern "C" void fn_801942C0(s32 primitive, Vertex* vertices, s32 count);

extern "C" void fn_1_144AC(void* context, Vec3* position, Vec2* size, TexCoords* texcoords)
{
	Color color = lbl_1_rodata_1A80;
	Vertex vertices[4];
