...
// GX library initialization, 0x801F0BE0 to 0x801F1F60. The pilot GX unit: it
// pins the GXData layout for the whole library. Transcribed from the Apr 2003
// Dolphin SDK (release build, no debugger -- the verify callbacks and write-
// gather helpers are inlined away). The three address-taken statics sit below
// the exported GXInit symbol, so GX .text starts at __GXDefaultTexRegionCallback.

// --- OS / PPC glue this unit needs (kept local, as the other dolphin TUs do) ---
...
extern void PPCMtwpar(u32 newwpar);

// --- entry points not covered by the GX headers ---

extern GXRenderModeObj GXNtsc480IntDf;
extern GXRenderModeObj GXPal528IntDf;
extern GXRenderModeObj GXEurgb60Hz480IntDf;
extern GXRenderModeObj GXMpal480IntDf;
extern VITVMode VIGetTvFormat(void);

// -----------------------------------------------------------------------------

const char* __GXVersion = "<< Dolphin SDK - GX\trelease build: Jul 23 2003 11:27:39 (0x2301) >>";
...
// These are function-local statics in the SDK source, but making them
// file-scope (as the reference SDK does, with the same comment) is what
// reproduces the DOL's .sbss order: MWCC reverse-emits these tentative globals
// as a group, landing __piReg..__memReg first and peCount..resetFuncRegistered
// after, exactly as in the object.
...
GXFifoObj* GXInit(void* base, u32 size)
{
...

	{
		...
		{
			u32 reg3 = 0;
			SOME_SET_REG_MACRO(reg3, 1, 0, 1);
			SOME_SET_REG_MACRO(reg3, 1, 1, 1);
			SOME_SET_REG_MACRO(reg3, 1, 2, 1);
			SOME_SET_REG_MACRO(reg3, 1, 3, 1);
			SOME_SET_REG_MACRO(reg3, 8, 24, 0x58);
			GX_WRITE_RAS_REG(reg3);
		}
	}
...
void __GXInitGX(void)
{
	...
	GXSetNumChans(0);
	GXSetChanCtrl(
	    GX_COLOR0A0, GX_DISABLE, GX_SRC_REG, GX_SRC_VTX, GX_LIGHT_NULL, GX_DF_NONE, GX_AF_NONE);
	GXSetChanAmbColor(GX_COLOR0A0, black);
	GXSetChanMatColor(GX_COLOR0A0, white);
	GXSetChanCtrl(
	    GX_COLOR1A1, GX_DISABLE, GX_SRC_REG, GX_SRC_VTX, GX_LIGHT_NULL, GX_DF_NONE, GX_AF_NONE);
