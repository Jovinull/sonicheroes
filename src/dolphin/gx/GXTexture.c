...
// GX2HWFiltConv sized to 8 (2 trailing zeros) reproduces the zero half-word the
// DOL keeps at the end of this TU's .sdata (dtk folds the linker-alignment pad
// into GX2HWFiltConv, sizing it 8); a 6-element array leaves our .sdata 2 bytes
// short (objdiff [.sdata-0] 98.4%). The linked DOL is byte-identical either way.
static u8 GX2HWFiltConv[8] = { 0x00, 0x04, 0x01, 0x05, 0x02, 0x06 };
    ...
    GXTexWrapMode wrap_s, GXTexWrapMode wrap_t, GXBool mipmap)
{
	...
	ASSERTMSGLINEV(654, ((u32)image_ptr & 0x1F) == 0, "%s: %s pointer not aligned to 32B",
	    "GXInitTexObj", "image");
...
void GXLoadTexObjPreLoaded(GXTexObj* obj, GXTexRegion* region, GXTexMapID id)
{
	...
	if (!(t->flags & 2)) {
		ASSERTMSGLINEV(1287, __GXData->tlutRegionCallback, "%s: Tex/Tlut Region Callback not set",
		    "GXLoadTexObj/PreLoaded");
		tlr = (__GXTlutRegionInt*)__GXData->tlutRegionCallback(t->tlutName);
		ASSERTMSGLINEV(
		    1289, tlr, "%s: Tex/Tlut Region Callback returns NULL", "GXLoadTexObj/PreLoaded");
...
void GXLoadTexObj(GXTexObj* obj, GXTexMapID id)
{
	...
	ASSERTMSGLINEV(
	    1324, __GXData->texRegionCallback, "%s: Tex/Tlut Region Callback not set", "GXLoadTexObj");
...
void GXInitTlutObj(GXTlutObj* tlut_obj, void* lut, GXTlutFmt fmt, u16 n_entries)
{
	...
	ASSERTMSGLINEV(
	    1354, n_entries <= 0x4000, "%s: number of entries exceeds maximum", "GXInitTlutObj");
	ASSERTMSGLINEV(
	    1356, ((u32)lut & 0x1F) == 0, "%s: %s pointer not aligned to 32B", "GXInitTlutObj", "Tlut");
	t->tlut = 0;
...
void GXLoadTlut(GXTlutObj* tlut_obj, u32 tlut_name)
{
	...
	ASSERTMSGLINEV(
	    1435, __GXData->tlutRegionCallback, "%s: Tex/Tlut Region Callback not set", "GXLoadTlut");
    ...
    GXTexCacheSize size_even, u32 tmem_odd, GXTexCacheSize size_odd)
{
	...
	ASSERTMSGLINEV(1488, (tmem_even & 0x1F) == 0, "%s: %s pointer not aligned to 32B",
	    "GXInitTexCacheRegion", "tmem even");
	ASSERTMSGLINEV(1490, (tmem_odd & 0x1F) == 0, "%s: %s pointer not aligned to 32B",
...
void GXInitTlutRegion(GXTlutRegion* region, u32 tmem_addr, GXTlutSize tlut_size)
{
	...
	ASSERTMSGLINEV(1655, (tmem_addr & 0x1FF) == 0, "%s: tmem pointer is not aligned to 512B",
	    "GXInitTlutRegion");
...
void GXSetTexCoordScaleManually(GXTexCoordID coord, GXBool enable, u16 ss, u16 ts)
{
	...
	ASSERTMSGLINEV(
	    1991, coord < GX_MAX_TEXCOORD, "%s: bad texcoord specified", "GXSetTexCoordScaleManually");
