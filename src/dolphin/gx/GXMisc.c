...
// GX pixel-engine init, draw-sync/draw-done, abort, and CPU->EFB pokes,
// 0x801F34B0 to 0x801F3B98. Transcribed from the Apr 2003 Dolphin SDK
// (release build, no debugger). This build's GXMisc is a reduced variant of the
// SDK source: __GXAbortWait/__GXAbortWaitPECopyDone are inlined into __GXAbort,
// GXDrawDone open-codes GXSetDrawDone + GXWaitDrawDone, and GXFlush is inlined
// into GXSetDrawSync while kept out-of-line for GXDrawDone.
...
static inline void SetDrawDone(void)
{
	BOOL enabled;
	u32 reg;
...
static inline void WaitDrawDone(void)
{
	BOOL enabled;
...
void GXPokeBlendMode(
    GXBlendMode type, GXBlendFactor src_factor, GXBlendFactor dst_factor, GXLogicOp op)
{
	u32 reg;
