...
// getters, the attenuation setters, GXInitSpecularDir*, and GXLoadLightObjIndx);
// transcribed from the Apr 2003 Dolphin SDK release build (no debugger, so the
// assert/verify helpers compile away and GXLoadLightObjImm uses PushLight).
    ...
    u32 light_mask, GXDiffuseFn diff_fn, GXAttnFn attn_fn)
{
	...
	ASSERTMSGLINE(
	    895, chan >= GX_COLOR0 && chan <= GX_COLOR1A1, "GXSetChanCtrl: Invalid Channel Id");
