...
// GXSetArray tracks indexBase/indexStride regardless of DEBUG; the shared
// __gx.h gates that on DEBUG, so restore the tracking form locally.
#undef GX_WRITE_SOME_REG2
...
void __GXCalculateVLim(void)
{
	...
	// tbl3 sized to 8 (4 trailing zeros) reproduces the zero word the DOL keeps
	// at the end of this TU's .sdata (split 0x8042C018-0x8042C028 = 0x10); a
	// 4-element tbl3 leaves our .sdata 4 bytes short (objdiff [.sdata-0] 85.7%).
	// The linked DOL is byte-identical either way.
...
void __GXSetVAT(void)
{
	...
	// The explicit (u8)i cast in the shift is load-bearing: without it (plain
	// `1 << i`) MWCC allocates the loop-invariant __GXData/offset registers
	// before the (u8)i loop-body scratch, coming out one register rotated from
	// the DOL (97.95%). The cast reproduces the oot/melee SDK source shape and
	// flips the allocation so the scratch takes the low register, matching 100%.
	for (i = 0; i < 8; i++) {
