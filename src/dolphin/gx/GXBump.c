...
// GX indirect-texture / bump-mapping setup, 0x801F612C to 0x801F6604.
// Transcribed from the Apr 2003 Dolphin SDK GXBump.c (release build, no
// debugger). This build's GXBump is a reduced variant of the SDK source: the
// GXSetTevIndWarp / GXSetTevIndTile / GXSetTevIndBumpST / GXSetTevIndBumpXYZ /
// GXSetTevIndRepeat helpers are unreferenced by the game and stripped, leaving
// the nine functions below in source order.
...
{
	// The .sdata2 pooled 1024.0f is followed by a zero word (linker-alignment
	// pad dtk folds into the constant, sizing it 8); as a named 2-element const
	// the second 0.0f reproduces it. A bare 1024.0f literal leaves our .sdata2
	// 4 bytes short (objdiff [.sdata2-0] 66.7%). DOL byte-identical either way.
	static const f32 c1024[2] = { 1024.0f, 0.0f };
	s32 mtx[6];
	u32 reg;
	u32 id;
	...
	mtx[0] = (int)(c1024[0] * offset[0][0]) & 0x7FF;
	mtx[1] = (int)(c1024[0] * offset[1][0]) & 0x7FF;
	scale_exp += 17;
	reg = 0;
	...
	mtx[2] = (int)(c1024[0] * offset[0][1]) & 0x7FF;
	mtx[3] = (int)(c1024[0] * offset[1][1]) & 0x7FF;
	reg    = 0;
	...
	mtx[4] = (int)(c1024[0] * offset[0][2]) & 0x7FF;
	mtx[5] = (int)(c1024[0] * offset[1][2]) & 0x7FF;
	reg    = 0;
...
void GXSetNumIndStages(u8 nIndStages)
{
	...
	ASSERTMSGLINE(
	    355, nIndStages <= 4, "GXSetNumIndStages: Exceeds max. number of indirect texture stages");
