...
// GXTev, 0x801F6604 to 0x801F6D68. The TEV register setters. Only the functions
// the game links survive here (dead-strip drops GXSetTevClampMode); transcribed
// from the Apr 2003 Dolphin SDK release build (no debugger, so the assert/verify
// helpers compile away).
...
void GXSetTevColorIn(
    GXTevStageID stage, GXTevColorArg a, GXTevColorArg b, GXTevColorArg c, GXTevColorArg d)
{
	u32 tevReg;
...
void GXSetTevAlphaIn(
    GXTevStageID stage, GXTevAlphaArg a, GXTevAlphaArg b, GXTevAlphaArg c, GXTevAlphaArg d)
{
	u32 tevReg;
...
void GXSetTevColor(GXTevRegID id, GXColor color)
{
	...
	regRA = 0;
	SET_REG_FIELD(745, regRA, 11, 0, color.r);
	SET_REG_FIELD(746, regRA, 11, 12, color.a);
	SET_REG_FIELD(747, regRA, 8, 24, 0xE0 + id * 2);

	regBG = 0;
	SET_REG_FIELD(749, regBG, 11, 0, color.b);
	SET_REG_FIELD(750, regBG, 11, 12, color.g);
	SET_REG_FIELD(751, regBG, 8, 24, 0xE1 + id * 2);
...
void GXSetTevColorS10(GXTevRegID id, GXColorS10 color)
{
	...
	ASSERTMSGLINE(777, color.r >= -1024 && color.r < 1024,
	    "GXSetTevColorS10: Color not in range -1024 to +1023");
	ASSERTMSGLINE(778, color.g >= -1024 && color.g < 1024,
	    "GXSetTevColorS10: Color not in range -1024 to +1023");
	ASSERTMSGLINE(779, color.b >= -1024 && color.b < 1024,
	    "GXSetTevColorS10: Color not in range -1024 to +1023");
	ASSERTMSGLINE(780, color.a >= -1024 && color.a < 1024,
	    "GXSetTevColorS10: Color not in range -1024 to +1023");

	CHECK_GXBEGIN(782, "GXSetTevColorS10");

	// The explicit `& 0x7FF` on each S10 component is load-bearing (oot/melee
	// SDK source shape): without it MWCC schedules the color loads and the
	// __GXData load differently from the DOL (~52%). Masking each s16 to its
	// 11-bit field before SET_REG_FIELD fixes the load scheduling to 100%.
	regRA = 0;
	SET_REG_FIELD(789, regRA, 11, 0, color.r & 0x7FF);
	SET_REG_FIELD(790, regRA, 11, 12, color.a & 0x7FF);
	SET_REG_FIELD(791, regRA, 8, 24, 0xE0 + id * 2);

	regBG = 0;
	SET_REG_FIELD(793, regBG, 11, 0, color.b & 0x7FF);
	SET_REG_FIELD(794, regBG, 11, 12, color.g & 0x7FF);
	SET_REG_FIELD(795, regBG, 8, 24, 0xE1 + id * 2);
...
void GXSetTevKColor(GXTevKColorID id, GXColor color)
{
	...
	regRA = 0;
	SET_REG_FIELD(838, regRA, 8, 0, color.r);
	SET_REG_FIELD(839, regRA, 8, 12, color.a);
	SET_REG_FIELD(840, regRA, 4, 20, 8);
	SET_REG_FIELD(841, regRA, 8, 24, 0xE0 + id * 2);

	regBG = 0;
	SET_REG_FIELD(843, regBG, 8, 0, color.b);
	SET_REG_FIELD(844, regBG, 8, 12, color.g);
	SET_REG_FIELD(845, regBG, 4, 20, 8);
	SET_REG_FIELD(846, regBG, 8, 24, 0xE1 + id * 2);
    ...
    GXTevColorChan blue, GXTevColorChan alpha)
{
	...
	ASSERTMSGLINE(
	    979, table < GX_MAX_TEVSWAP, "GXSetTevSwapModeTable: Invalid Swap Selection Index");
...
void GXSetTevOrder(GXTevStageID stage, GXTexCoordID coord, GXTexMapID map, GXChannelID color)
{
	...
	u32 tmap;
	u32 tcoord;
	// The 10th (unused) element reproduces the trailing zero word the DOL keeps
	// at the end of this TU's .data (split range 0x802989A0-0x80298A18 = 0x78);
	// with a 9-element array our object's .data is 4 bytes short (objdiff
	// [.data-0] 98.3%). The linked DOL is byte-identical either way.
	static int c2r[10] = { 0, 1, 0, 1, 0, 1, 7, 5, 6 };

	CHECK_GXBEGIN(1131, "GXSetTevOrder");
	ASSERTMSGLINE(1132, stage < GX_MAX_TEVSTAGE, "GXSetTevOrder: Invalid Tev Stage Index");
	ASSERTMSGLINE(1134, coord < GX_MAX_TEXCOORD || coord == GX_TEXCOORD_NULL,
	    "GXSetTevOrder: Invalid Texcoord");
	ASSERTMSGLINE(1136, (map & ~GX_TEX_DISABLE) < GX_MAX_TEXMAP || map == GX_TEXMAP_NULL,
	    "GXSetTevOrder: Invalid Tex Map");
	ASSERTMSGLINE(1138, color >= GX_COLOR0A0 && color <= GX_COLOR_NULL,
	    "GXSetTevOrder: Invalid Color Channel ID");
...
void GXSetNumTevStages(u8 nStages)
{
	...
	ASSERTMSGLINE(
	    1189, nStages != 0 && nStages <= 16, "GXSetNumTevStages: Exceed max number of tex stages");
