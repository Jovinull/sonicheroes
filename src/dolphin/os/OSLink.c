...
#define OS_SECTIONINFO_OFFSET(x) ((x) & ~0x1)
...
extern OSModuleQueue __OSModuleList : 0x800030C8;
extern void* __OSStringTable : 0x800030D0;
#else
...
extern void DCFlushRange(void* start, u32 size);
...
BOOL OSUnlink(OSModuleInfo* oldModule)
{
	...
	if (moduleHeader->prologSection != SHN_UNDEF) {
		moduleHeader->prolog -= OS_SECTIONINFO_OFFSET(
		    OSGetSectionInfo(oldModule)[moduleHeader->prologSection].offset);
	}

	if (moduleHeader->epilogSection != SHN_UNDEF) {
		moduleHeader->epilog -= OS_SECTIONINFO_OFFSET(
		    OSGetSectionInfo(oldModule)[moduleHeader->epilogSection].offset);
	}
